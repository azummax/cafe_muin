...
include_once('./admin.shop.lib.php');     // 375line order_update_receipt 함수 정의
include_once(G5_LIB_PATH.'/mailer.lib.php');   // 입금확인 메일 mailer 함수 정의
...
//================ 입금완료 상태로 변경 시 처리사항(입금액/입금날짜 업데이트, 주문자에게 알림톡발송) 추가 =====================//
if($_POST['ct_status']=="입금" && ($info['od_settle_case'] == '무통장')){

	$sql = " select * from {$g5['g5_shop_order_table']} where od_id = '$od_id' ";
    $od = sql_fetch($sql);

	if($od['od_misu'] > 0 && $od['od_receipt_price'] == 0){
			...
			$od_send_alimtalk = true;
			$od_send_alimtalk2 = true;

			//include "./orderalimtalk.inc.php";
			//include "./orderalimtalk2.inc.php";


		}

}
//================ 입금완료 상태로 변경 시 처리사항(입금액/입금날짜 업데이트, 주문자에게 알림톡발송) 끝=====================//
