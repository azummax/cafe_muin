...
//주문금액 구하기
$sql  = " select	sum(od_cart_price + od_send_cost +od_send_cost2) as tot_cart_price  
...
<div  style="margin: -10px 0 10px;padding:10px 20px; " >
<table cellpadding="0" cellspacing="0" border="0" class="boxContainer" >
	<tr>
		<td class="left"  style="padding:3px"><strong>주문리스트</strong>  |
			<span class="bul">검색결과 : </span><span class="fc_red"><strong><?php echo $total_count; ?></strong>건</span>
			<span class="bul">총주문금액 : </span><span class="fc_red"><strong><?php echo number_format($data['tot_cart_price']);?>원</strong></span></td>
...
<div class="tbl_head01 tbl_wrap">
    ...
    <tr>
		...
		<th scope="col" id="th_odrer"   >상품명</th>
        ...
        <th scope="col" >주문금액<br>배송비포함</th>
        <th scope="col" >할인금액(쿠폰)</th>
		...
		<th scope="col" id="odrpay">결제일</th>
		<th scope="col" id="odrpay">메모</th>
        <th scope="col" >보기</th>
    </tr>
	<!--
    <tr>
    ...
    for ($i=0; $row=sql_fetch_array($result); $i++)
    {
        ...
        if ($row['od_settle_case'])
        {
			...
			if($s_receipt_way == "네이버페이") $s_receipt_way = "<span style='color:#33cc00'>네이버페이</span>";
        ...
        if ($row['od_receipt_point'] > 0)
            $s_receipt_way .= $s_br."M포인트";

		if ($row['use_event_point'] > 0)
            $s_receipt_way .= $s_br."EV포인트";
		...
		//주문 상품명 구하기
		$goods = get_goods($row['od_id']);
		$item_name = $goods['full_name'];


		$od_receipt_time = substr($row['od_receipt_time'],0,10);
    ...
    <tr class="orderlist<?php echo ' '.$bg; ?>">
		...
		<td headers="th_recvr" style="width:250px; text-align:left;"><?php echo $item_name; ?></td>
        ...
        <td headers="th_recvr" class="td_name">
			<a href="<?php echo $_SERVER['SCRIPT_NAME']; ?>?sort1=<?php echo $sort1; ?>&amp;sort2=<?php echo $sort2; ?>&amp;sel_field=od_b_name&amp;search=<?php echo get_text($row['od_b_name']); ?>"><?php echo get_text($row['od_b_name']); ?></a>
		</td>
		...
		<?
		$is_nodelivery = "";
		if(($row['od_status'] == "배송" || $row['od_status'] == "완료" ) && $row['od_cart_count'] > 1){
			//미배송 주문건
			$cart = sql_fetch(" select COUNT(ct_id) as cartCnt from {$g5['g5_shop_cart_table']} where od_id = '{$row['od_id']}' and ct_status = '입금' ");
			$noDeliveryCnt =$cart['cartCnt']; 


			$is_nodelivery =$noDeliveryCnt;


		}
		?>
		
		<td headers="th_odrcnt" style="width:70px"><?php echo $row['od_cart_count']; ?>건  <?php if($is_nodelivery) echo "<br><span style='font-size:8px; color:red'>미배송 : ".$noDeliveryCnt."건</span>"; ?></td>
        <td  class="td_num "><?php echo number_format($row['od_cart_price'] + $row['od_send_cost'] + $row['od_send_cost2']); ?></td>
        <td  class="td_num_right"  style="width:70px"><?php echo number_format($row['couponprice']+$row['order_event_point']); ?></td>
        <td  class="td_num_right"style="width:50px"><?php echo number_format($row['od_receipt_point']); ?></td>
        <td  class="td_num_right td_numsum" style="width:60px">
		...
		<td  headers="odrstat" class="odrstat"  style="width:50px">
            <input type="hidden" name="current_status[<?php echo $i ?>]" value="<?php echo $row['od_status'] ?>">
            <?php 
				if($row['od_status'] == "취소")			
					echo "<font color=red>".$row['od_status']."</font>"; 
				else if($row['od_status'] == "배송")		
					echo "<font color=blue>".$row['od_status']."</font>"; 
				else if($row['od_status'] == "완료")		
					echo "<font color=#339900>".$row['od_status']."</font>"; 
				else														
					echo $row['od_status']; 
			//echo $row['od_status']; 
			?>
        </td>
        <td   headers="odrpay" style="width:70px">
            <input type="hidden" name="current_settle_case[<?php echo $i ?>]" value="<?php echo $row['od_settle_case'] ?>">
            <?php echo $s_receipt_way; ?>
			<?php if($row['pt_memo']) echo "<br><font color='red'>수기결제</font>"; ?>
        </td>
		<td headers="odrpay" class="odrpay"  style="width:70px">
			<?php echo $od_receipt_time; ?>
		</td>
		<td headers="odrpay" class="odrpay"  >
			<?php echo $row['od_memo']; ?>
		</td>
    ...
    <?php
        ...
        $tot_pointprice   += $row['od_receipt_point'];
        $tot_epointprice   += $row['use_event_point'];
    ...
    <tr class="orderlist">
        ...
        <td><?php echo number_format($tot_pointprice); ?></td>
