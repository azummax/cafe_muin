...
if($w == '' && $default['de_member_reg_coupon_use'] && $default['de_member_reg_coupon_term'] > 0 && $default['de_member_reg_coupon_price'] > 0) {
    ...
    if($create_coupon) {
        $cp_subject = '[신규 회원가입 쿠폰] 10,000원 할인';
        $cp_method = 2; //0: 개별상품할인, 1: 카테고리할인, 2:주문금액할인, 3:배송비할인
        $cp_target = '';
        $cp_start = G5_TIME_YMD;
        $cp_end = date("Y-m-d", (G5_SERVER_TIME + (86400 * ((int)$default['de_member_reg_coupon_term'] - 1))));
        $cp_type = 0; // 0 : 정액할인(원), 1: 정률할인(%)
    ...
    if($create_coupon) {
		$delivery_cp_subject = '[첫 구매 혜택] 무료배송 쿠폰';
        $delivery_cp_method = 3; //0: 개별상품할인, 1: 카테고리할인, 2:주문금액할인, 3:배송비할인
        $delivery_cp_target = '';
        $delivery_cp_start = G5_TIME_YMD;
        $delivery_cp_end = date("Y-m-d", (G5_SERVER_TIME + (86400 * ((int)$default['de_member_reg_coupon_term'] - 1))));
        $delivery_cp_type = 0; // 0 : 정액할인(원), 1: 정률할인(%)
        $delivery_cp_price = "3000";
        $delivery_cp_trunc = 1;  //절사금액(1: 1원단위, 10:10원단위, 100:100원단위, 1000:1000원 단위)
        $delivery_cp_minimum = $default['de_member_reg_coupon_minimum'];
        $delivery_cp_maximum = 0;
		
		 $sql = " INSERT INTO {$g5['g5_shop_coupon_table']}
                    ( cp_id, cp_subject, cp_method, cp_target, mb_id, cp_start, cp_end, cp_type, cp_price, cp_trunc, cp_minimum, cp_maximum, cp_datetime )
                VALUES
                    ( '$cp_id', '$delivery_cp_subject', '$delivery_cp_method', '$delivery_cp_target', '$mb_id', '$delivery_cp_start', '$delivery_cp_end', '$delivery_cp_type', '$delivery_cp_price', '$delivery_cp_trunc', '$delivery_cp_minimum', '$delivery_cp_maximum', '".G5_TIME_YMDHIS."' ) ";

        $res = sql_query($sql, false);
		
    }
	//===================신규회원가입 무료배송 쿠폰발행 End=============================//
