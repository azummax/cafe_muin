...
function apms_get_file($dir, $pf_id) {
    ...
    $result = sql_query($sql);
	$nonce = download_file_nonce_key('item', $pf_id);
	while ($row = sql_fetch_array($result)) {
        $no = $row['pf_no'];
        $file[$no]['id'] = $pf_id;
		$file[$no]['href'] = $pf_url."/download.php?pf_id=$pf_id&amp;pf=$pf_dir&amp;no=$no&amp;nonce=$nonce";
        $file[$no]['href_view'] = $pf_url."/view.php?pf_id=$pf_id&amp;pf=$pf_dir&amp;no=$no&amp;nonce=$nonce";
...
function apms_item_rows($arr) {
	...
	$sql_common = "from {$g5['g5_shop_item_table']} where it_use = '1' $sql_type $sql_ca $sql_ca_ex $sql_pt $sql_term $sql_main $sql_where";
	if($page > 1) {
