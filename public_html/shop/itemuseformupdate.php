...
if ($w == "") {
    ...
    */
    ...
    if ($default['de_item_use_use']) {
		...
		// 사용자 후기작성시 포인트 쌓기 시작
		$itit = sql_fetch(" select it_name from g5_shop_item where it_id = '$it_id' ");	// 상품조회
		$point = $config['cf_1'];				// 일반리뷰 포인트
		$point_img = $config['cf_2'];		// 포토리뷰 포인트

		if($is_photo){		// 포토리뷰 작성시
			insert_point($member['mb_id'], $point_img, $itit['it_name'].' 상품 포토리뷰 작성', '@review', $member['mb_id']."/".$it_id, '리뷰작성');
		}else{																					// 일반리뷰 작성시
			insert_point($member['mb_id'], $point, $itit['it_name'].' 상품 일반리뷰 작성', '@review', $member['mb_id']."/".$it_id, '리뷰작성');
		}
		// 사용자 후기작성시 포인트 쌓기 끝
   }
