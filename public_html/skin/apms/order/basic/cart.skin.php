...
<form name="frmcartlist" id="sod_bsk_list" method="post" action="<?php echo $action_url; ?>" class="form" role="form">
			...
			<tr<?php echo ($i == 0) ? ' class="tr-line"' : '';?>>
				<td class="text-center">
					<div class="order_ck">
						...
						<label for="ct_chk_<?php echo $i; ?>">상품</label>
					</div>
				</td>
				<td class="text-center img_box">
					<div class="item-img">
						<?php echo get_it_image($item[$i]['it_id'], 140, 140); ?>
						<!-- <div class="item-type">
				...
				<td>
					...
					<a href="./item.php?it_id=<?php echo $item[$i]['it_id'];?>">
						<strong class="item_name"><?php echo stripslashes($item[$i]['it_name']); ?></strong>
					</a>
					<?php if($item[$i]['it_options']) { ?>
						<div class="item_option"><?php echo $item[$i]['it_options'];?></div>
						<button type="button" class="mod_options option_modify">선택옵션수정</button>
					<?php } ?>
				</td>
				<td class="text-center"><?php echo number_format($item[$i]['qty']); ?></td>
				<td class="text-center"><?php echo number_format($item[$i]['ct_price']); ?></td>
				<td class="text-center"><span id="sell_price_<?php echo $i; ?>"><?php echo number_format($item[$i]['sell_price']); ?></span></td>
				<td class="text-center"><?php echo number_format($item[$i]['point']); ?></td>
	...
	<button type="button" onclick="return form_check('seldelete');" class="cart_delete">선택삭제</button>

    <?php if ($tot_price > 0 || $send_cost > 0) { ?>
		<div id="cart_total_box">
			<div>
				<b>총 주문금액</b>
				<strong><b><?php echo number_format($tot_price - $send_cost); ?></b>원</strong>
			</div>
			<div>
				<img src="/thema/Basic/img/cart_total_box_ico01.png" alt="+" />
				<b>총 배송비</b>
				<strong><b><?php echo( $send_cost > 0 ) ? number_format($send_cost):'0'; //배송비 ?></b>원</strong>
			</div>
			<div>
				<img src="/thema/Basic/img/cart_total_box_ico02.png" alt="=" />
				<b>결제예정금액</b>
				<strong><b><?php echo( $tot_price > 0 ) ? number_format($tot_price):'-'; //총금액 ?></b>원</strong>
			</div>
		</div>
	<?php } ?>
    ...
    <div style="margin-bottom:15px; text-align:center;">
        <?php if ($i == 0) { ?>
	        <a href="<?php echo G5_SHOP_URL; ?>/" class="order_btn" style="margin-top:20px;">계속하기</a>
        <?php } else { ?>
			...
			<div class="order_btn_box">
				<a href="<?php echo G5_SHOP_URL; ?>/list.php?ca_id=<?php echo $continue_ca_id; ?>" class="order_btn">계속하기</a>
				<button type="button" onclick="return form_check('buy');" class="order_btn on">상품주문</button>
			</div>
