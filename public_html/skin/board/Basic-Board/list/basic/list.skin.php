...
// 출력설정
$num_notice = ($is_thumb) ? '*' : '<b class="list_notice">공지</b>';
...
<div class="list-board">
	<div class="div-head <?php echo $head_class;?>">
		...
		<?php } ?>
		<?php if($is_date) { ?>
			<span class="wr-date hidden-xs">등록일</span>
		<?php } ?>
		<?php if($is_hit) { ?>
			<span class="wr-hit hidden-xs">조회수</span>
		<?php } ?>
		<?php if($is_down) { ?>
	...
	for ($i=0; $i < $list_cnt; $i++) { 
		...
		if ($list[$i]['wr_link1']) {
			//$wr_icon .= '<span class="wr-icon wr-link"></span>';
		}
	...
	?>
		<li class="list-item<?php echo $li_css;?>">
			...
			<div class="wr-subject">
				<a href="<?php echo $list[$i]['href']; ?>" class="ellipsis"<?php echo $list[$i]['target'];?><?php echo $is_modal_js;?>>
