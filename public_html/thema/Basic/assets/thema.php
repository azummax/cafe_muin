...
$add_stylesheet .= PHP_EOL . '<link rel="stylesheet" href="' . COLORSET_URL . '/colorset.css?ver=' . date("YmdHis") . '" type="text/css" class="thema-colorset">';
$add_stylesheet .= PHP_EOL . '<link rel="stylesheet" href="' . THEMA_URL . '/assets/css/cafe-muen.css?ver=' . time() . '" type="text/css">';
...
if (!$page_background) {
	...
	if (isset($at_set['background']) && $at_set['background'] && $at_set['background'] != "none") { //배경이미지
		...
		if (isset($at_set['bg']) && $at_set['bg']) { //중앙,상단,하단,패턴
			switch ($at_set['bg']) {
				case "top":
					$page_background .= "background-position: center top;";
					break;
				case "bottom":
					$page_background .= "background-position: center bottom;";
					break;
				case "fixed":
					$page_background .= "background-size:100%; background-position: center top; background-attachment:scroll;";
					break;
				case "pattern":
					$page_background .= "background-repeat: repeat; background-size:auto; background-attachment:scroll;";
					break;
			}
		}
	}
...
<style>
	...
	<?php } ?>
	.at-container {
		max-width: <?php echo $is_thema_size; ?>px;
	}

	.no-responsive .wrapper,
	.no-responsive .at-container-wide {
		min-width: <?php echo $is_thema_size; ?>px;
	}

	.no-responsive .boxed.wrapper,
	.no-responsive .at-container {
		width: <?php echo $is_thema_size; ?>px;
	}

	.at-menu .nav-height {
		height: <?php echo $is_menuh; ?>px;
		line-height: <?php echo $is_menuh; ?>px !important;
	}
